...
//    private final MatchPlanReturnDTOService matchPlanReturnDTOService;
    ...
    public GameFromTeamReturnDTO gameToGameFromTeamReturnDTO (Game game){
        if (game == null)
            return null;
        return new GameFromTeamReturnDTO(game.getId(), game.getSwitchGame(), roundToRoundFromGameFromRegistrationReturnDTO(game.getRound()), game.getPoints().stream().map(this::pointsToPointsFromGameFromTeamReturnDTO).collect(Collectors.toSet()));
    }

    public RoundFromGameFromTeamReturnDTO roundToRoundFromGameFromRegistrationReturnDTO (Round round){
        if (round == null)
            return null;
        return new RoundFromGameFromTeamReturnDTO(round.getId(), round.getStartTime(), round.getEndTime(), round.isFinalGame(), round.isPlayed());
    }

    public PointsFromGameFromTeamReturnDTO pointsToPointsFromGameFromTeamReturnDTO (Points points){
        if (points == null)
            return null;
        return new PointsFromGameFromTeamReturnDTO(points.getId(), Math.max(points.getGroupPoints(), points.getFinalPoints()));
    }
