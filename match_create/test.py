...
def generate_schedule(num_teams=25, num_fields=4, num_rounds=8, teams_per_game=4):
    ...
    schedule = {f"Round {i+1}": {f"Field {chr(65+j)}": [] for j in range(num_fields)} for i in range(num_rounds)}
    ...
    for rnd in range(num_rounds):
        ...
        for fld in range(num_fields):
            game_teams = round_teams[fld*teams_per_game:fld*teams_per_game+teams_per_game]
